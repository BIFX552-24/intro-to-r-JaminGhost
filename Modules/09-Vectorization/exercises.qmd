...
m\^-1 you will get the matrix with values less than 1 , by dividing all the values in my by 1

m\* c(1,0,-1) you will get the matrix list multiplied by the vector list 1, 0, and -1, so row 1 is muliplied by 1 , row two is multiplied by 0, and row 3 multiplied by -1

m\>c(0,20) you will get a list of values less than 0 and 20 in the matrix outputting false or true if less than 0 or 20.

m\* c(1,0,-1,2) the matrix list will go in order of the vector by multiplying 1 with the first value in the matrix, and then 0 in the second value in the matrix, and so on until it reaches the end of the vector and repeats the vector list and reaches the end of the matrix
