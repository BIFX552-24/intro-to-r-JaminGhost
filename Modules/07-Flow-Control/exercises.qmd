...
The values between output_vector and output_vector2 are not the same. They have the same total values of 25.
...
{
  mean_lifExp<-gapminder[gapminder$continent == i, 'lifeExp']%>%
    unlist()%>%
    mean()
  if(mean_lifExp >= 50)
  {
    ...
    paste(i, 'has a mean life expectacy less than 50 years')%>%
      print()
  }
}
```
...
{
  mean_lifExp<-gapminder[gapminder$country == i, 'lifeExp']%>%
    unlist()%>%
    mean()
  ...
  if(mean_lifExp < 50)
  {
    ...
    print()
  }else if(mean_lifExp <70) {
    paste(i, 'has a mean life expectacy between 70 and 50 years')%>%
      print()
}else{
